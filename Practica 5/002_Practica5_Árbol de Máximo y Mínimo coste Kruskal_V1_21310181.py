...
#Planificación de Redes Sociales y Eventos
import heapq
...
def imprimir_resultado(mensaje, resultado):
    ...
    for origen, destino, peso in resultado:
        print(f"Ruta ({origen} - {destino}) con distancia {peso} metros")
...
def main():
    ...
    # Agregar aristas al grafo con nombres de puntos de interés en un evento
    grafo.agregar_arista('Escenario Principal', 'Puesto de Comida', 10)
    grafo.agregar_arista('Escenario Principal', 'Área de Descanso', 15)
    grafo.agregar_arista('Escenario Principal', 'Zona de Juegos', 20)
    grafo.agregar_arista('Puesto de Comida', 'Área de Descanso', 5)
    grafo.agregar_arista('Área de Descanso', 'Zona de Juegos', 10)
    grafo.agregar_arista('Zona de Juegos', 'Entrada Principal', 25)
    grafo.agregar_arista('Entrada Principal', 'Puesto de Comida', 30)
    grafo.agregar_arista('Entrada Principal', 'Área de Descanso', 35)

    # Ejecutar el algoritmo de Kruskal para encontrar la ruta óptima
    print("Ejecutando Kruskal para encontrar la ruta óptima:")
    mst_minimo = grafo.kruskal()

    # Imprimir el resultado del Árbol de Expansión Mínima
    imprimir_resultado("Ruta óptima:", mst_minimo)
